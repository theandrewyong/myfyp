<?php
	...
	if(isset($_POST["submit"])){
		...
		//To count how many employees added in table
		...
		$counter = $count_result["total"] + 1;
		 ...
		 //Count must be dynamic
		for($i=0;$i<$counter;$i++){
			//Ff employee checkbox is checked where cb + number is checkbox name
			if(isset($_POST["cb$i"])){
				...
				while($ef = mysqli_fetch_assoc($epf_formula_sql)){
					...
					$adhoc_amt = $get_specific_result["adhoc_amt"]; 
